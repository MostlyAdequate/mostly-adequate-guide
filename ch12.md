...
traverse(of, fn) {
    return this.$value.reduce(
      (listInContainer, item) => fn(item).map(b => bs => bs.concat(b)).ap(listInContainer),
      of(new List([])),
    );
  }
```

This just runs a `reduce` on the list. The seed value is `of(new List([]))`, which in our case is `Right([])`. The reduce function is `(listInContainer, item) => fn(item).map(b => bs => bs.concat(b)).ap(listInContainer)`, which looks a bit scary, so let's step through it.
...
- When Right, `Either.map` passes the Right value to the function and returns a new Right with the result. In this case the function has two parameters (`b` and `bs`), and is curried, so a one parameter function is returned. 
  - `Right(bs => bs.concat(b))`
- When Left, `Either.map` returns the Left value
  - `Left(a)`

`.ap(listInContainer)`
- When Right, `Either.ap` returns `newList.map(this.$value)`. `listInContainer` starts life as `Right([])`. 
  - When listInContainer is `Right` map calls `bs => bs.concat(b)`, which returns a `Right` with the item added to the list.
    - `Right([as])`
  - When listInContainer is `Left`, map just returns the Left value from a previous iteration.
    - `Left(a)`
- When Left, `Either.ap` returns the Left value from the previous step.
  - `Left(a)`
