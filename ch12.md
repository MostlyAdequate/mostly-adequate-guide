...
Let's start with the second argument. It must be a *Traversable* holding an *Applicative*, which sounds quite restrictive, but just so happens to be the case more often than not. It is the `t (f a)` which gets turned into a `f (t a)`. Isn't that expressive? It's clear as day the two types do-si-do around each other. That first argument there is merely a crutch and only necessary in an untyped language. It is a type constructor (our *point*) provided so that we can invert map-reluctant types like `Left` - more on that in a minute.
...
const maybeToEither = x => x ? Right(x.__value) : Left()

natLaw1(Right, idToEither)(Identity(Maybe.of('barlow one')))
// Right(Identity('barlow one'))

natLaw2(Id, idToEither)(Identity(Maybe.of('barlow one')))
// Right(Identity('barlow one'))
```
